...
console.log('\ndictionary')
{
  ...
  // Casino vocabulary validation (mirrors src/dictionary.ts lookup logic)
  const customSrc = readFileSync(resolve(ROOT, 'src/data/custom-words.ts'), 'utf8')
  const customList = eval(customSrc.replace(/export const CUSTOM_WORDS: string\[\] =/, 'const words =') + '\nwords')
  const customSet = new Set(customList.map((w) => w.toLowerCase()))
  const isWordWithCustom = (w) => customSet.has(w.toLowerCase()) || isWord(w)
  ...
  for (const w of [
    ...
    'monster', 'pocket', 'jackpot', 'wager', 'poker', 'casino', 'slots', 'vault'
  ]) {
    check(`casino word "${w}" is recognized`, isWordWithCustom(w))
  }
