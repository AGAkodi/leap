...
function buildCenter() {
  ...
  // Four grand Casino Pyramid Monuments at the plaza corners — luxury stepped
  // pedestals with gold trims, neon accent pillars, and iconic casino crowns
  // (Dice, Slots, Roulette, Vault). Replaces the old Aztec stone pyramids.
  const cornerLandmarks = [
    { x: x0 + 8, z: z0 + 8, type: 'dice', name: 'Casino Pyramid (Dice Tower)' },
    { x: x0 + BLOCK - 8, z: z0 + 8, type: 'slots', name: 'Casino Pyramid (Slots Tower)' },
    { x: x0 + BLOCK - 8, z: z0 + BLOCK - 8, type: 'roulette', name: 'Casino Pyramid (Roulette Spire)' },
    { x: x0 + 8, z: z0 + BLOCK - 8, type: 'vault', name: 'Casino Pyramid (Vault Citadel)' }
  ]

  for (const c of cornerLandmarks) {
    const px = c.x
    const pz = c.z
    // Base tier — dark mahogany with gold trim (footprint 12m)
    add({
      name: `${c.name} Base`,
      pos: [px, 0.6, pz],
      scale: [12, 1.2, 12],
    ...
    add({
      name: `${c.name} Gold Rim 1`,
      pos: [px, 1.25, pz],
      scale: [12.2, 0.15, 12.2],
      color: C.gold,
      metallic: 0.7,
      roughness: 0.2,
      collider: 0
    })
    // Mid tier — deep casino red felt
    add({
      name: `${c.name} Mid`,
      pos: [px, 1.9, pz],
      scale: [9, 1.2, 9],
    ...
    // Neon corner accent pillars
    for (let dx = -1; dx <= 1; dx += 2) {
      for (let dz = -1; dz <= 1; dz += 2) {
        add({
          name: `${c.name} Neon Pillar`,
          pos: [px + dx * 2.8, 4.2, pz + dz * 2.8],
          scale: [0.35, 1.8, 0.35],
          mesh: 'cylinder',
          color: dx === dz ? C.neonPink : C.neonCyan,
          emissive: dx === dz ? C.neonPink : C.neonCyan,
          emissiveIntensity: 1.8,
          collider: 0
        })
      }
    }
    // Crown set-piece atop the pyramid
    if (c.type === 'dice') {
      add({
        name: `${c.name} Giant Die`,
        pos: [px, 4.9, pz],
        scale: [2.6, 2.6, 2.6],
        rot: yawQuat(25),
        color: C.chipWhite,
    ...
    } else if (c.type === 'slots') {
      add({
        name: `${c.name} Slot Monolith`,
        pos: [px, 4.9, pz],
        scale: [2.4, 2.6, 2.4],
        color: C.chipBlack,
        roughness: 0.3,
    ...
    } else if (c.type === 'roulette') {
      add({
        name: `${c.name} Roulette Column`,
        pos: [px, 4.7, pz],
        scale: [2.2, 1.8, 2.2],
        mesh: 'cylinder',
        color: C.woodDark,
  ...
  // Four grand Casino Gateway Portals aligned with the four gameplay zones
  // Replaces the old mossy stone arch.glb with sleek gold & neon marquees.
  const gates = [
    { name: 'Gate North (The Vault)', x: cx, z: z0 + BLOCK - 1.5, yaw: 0, neon: C.neonCyan },
    { name: 'Gate South (Slots Hall)', x: cx, z: z0 + 1.5, yaw: 0, neon: C.neonPink },
    { name: 'Gate East (Roulette Pit)', x: x0 + BLOCK - 1.5, z: cz, yaw: 90, neon: C.chipRed },
    { name: 'Gate West (Poker Lounge)', x: x0 + 1.5, z: cz, yaw: 90, neon: C.feltGreen }
  ]
  ...
  for (const g of gates) {
    ...
    // Left pillar
    add({
      name: `${g.name} Pillar L`,
      pos: [isEW ? g.x : g.x - colOffset, 2.5, isEW ? g.z - colOffset : g.z],
      scale: [0.9, 5.0, 0.9],
      mesh: 'cylinder',
      color: C.aztecDark,
      metallic: 0.4,
    ...
    // Overhead Gold Marquee Beam
    add({
      name: `${g.name} Marquee Beam`,
      pos: [g.x, 5.3, g.z],
      scale: isEW ? [1.2, 0.8, 6.4] : [6.4, 0.8, 1.2],
      color: C.gold,
      metallic: 0.7,
      roughness: 0.2,
      collider: 0
    ...
    add({
      name: `${g.name} Neon Strip`,
      pos: [g.x, 4.85, g.z],
      scale: isEW ? [0.6, 0.1, 5.8] : [5.8, 0.1, 0.6],
      color: g.neon,
      emissive: g.neon,
      emissiveIntensity: 2.0,
      collider: 0
    })
    // Floating zone-name Sign
    add({
      name: `${g.name} Sign`,
      pos: [g.x, 6.2, g.z],
      scale: [1, 1, 1],
...
function buildCasinoLandmarks(x0, z0, cx, cz) {
  ...
  // (assets/models/roulette_table.glb).
  {
    const wx = x0 + BLOCK - 4
    const wz = cz + 8
    const id = add({
      ...
      collider: 0 // collision comes from the model's own visible mesh
    })
  ...
  // (assets/models/slot_machine.glb) in a row.
  {
    const sz = z0 + 4
    for (let i = 0; i < 3; i++) {
